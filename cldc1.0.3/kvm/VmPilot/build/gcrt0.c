...
start ()
{
	...
	if (SysAppStartup (&appInfo, &prevGlobals, &globalsPtr) != 0) {
		...
		if ((mainCmd == sysAppLaunchCmdNormalLaunch || 
			 mainCmd == sysAppLaunchCmdGoTo) && FrmAlert(1999)) { 
...
do_bhook(Word cmd, Ptr PBP, Word flags)
{
	...
	hookptr = GET_FAR_VALUE_PC_RELATIVE(bhook_start);
	hookend = GET_FAR_VALUE_PC_RELATIVE(bhook_end);
...
do_ehook(Word cmd, Ptr PBP, Word flags)
{
	...
	hookstart = GET_FAR_VALUE_PC_RELATIVE(ehook_start);
	hookptr = GET_FAR_VALUE_PC_RELATIVE(ehook_end);
