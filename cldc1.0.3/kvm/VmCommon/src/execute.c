...
/*=======================================================================*/
        ...
        callMethod_general: {
            ...
            if (thisMethod->accessFlags & ACC_ABSTRACT) {
                ...
                VMSAVE
                raiseException(AbstractMethodError);
                VMRESTORE
            }
...
/*=======================================================================*/
    ...
    */
        ...
        /*
         * This *must* have been called within a VMSAVE/VMRESTORE pair
         * for throwException to get the right values for IP, FP etc.
         * as all non-volatile variables in the TRY body are undefined in
         * the CATCH block.
         */
#if INCLUDEDEBUGCODE
        /*
         * VMRESTORE in debug mode clears the global registers so this is
         * one means for testing whether or not VMSAVE was called. Note
         * that we can't test the values of the local registers as they
         * are volatile under setjmp/longjmp.
         */
        if ((IPISLOCAL && ip_global == 0) ||
            (SPISLOCAL && sp_global == 0) ||
            (LPISLOCAL && lp_global == 0) ||
            (FPISLOCAL && fp_global == 0) ||
            (CPISLOCAL && cp_global == 0))
            fatalVMError("VMSAVE not called");
        ...
        END_TEMPORARY_ROOTS
        /*
         * Now restore the local variables.
         */
        VMRESTORE
