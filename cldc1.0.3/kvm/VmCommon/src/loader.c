...
#if SVM
/* This is used to do the necessary type casting when indexing into the
 * TrustedCP.
 */
#define TRUSTEDCP(i)        (*((TRUSTEDCPENTRY*)(&TrustedCP->data[i])))
#define TRUSTEDCP_STRING(i) ((char*)TRUSTEDCP(i)->data)
#endif
...
#if SVM
#define RECOGNIZED_TRUSTED_METHOD_FLAGS (TACC_UCLASS_CAN_INVOKE)
#define RECOGNIZED_TRUSTED_CLASS_FLAGS (TACC_SUBCLASS | \
                     TACC_CLASS_RESOURCE_ACCESS | \
                     TACC_EXCEPTION )
#endif /* SVM */
...
getUTF8String(POINTERLIST_HANDLE StringPoolH, unsigned short index)
{
    ...
    if (index >= StringPool->length ||
        ...
        raiseExceptionCharMsg(ClassFormatError,KVM_MSG_BAD_UTF8_INDEX);
    }
 ...
 *   returns:     nothing
 *   throws:      ClassFormatError if the UTF8 string is invalid
 *=======================================================================*/
...
verifyUTF8String(char* bytes, unsigned short length)
{
 ...
 failed:
    raiseExceptionCharMsg(ClassFormatError,KVM_MSG_BAD_UTF8_STRING);
}
 ...
 *   returns:     nothing
 *   throws:      ClassFormatError if name is invalid
 *=======================================================================*/
...
static void verifyName(const char* name, enum validName_type type)
{
    if (!isValidName(name,type)) { 
        raiseExceptionCharMsg(ClassFormatError,KVM_MSG_BAD_NAME);
    } 
}
 ...
 *   returns:     nothing
 *   throws:      ClassFormatError if flags are invalid.
 *=======================================================================*/
...
verifyClassFlags(unsigned short flags)
{
 ...
 failed:
    raiseExceptionCharMsg(ClassFormatError,KVM_MSG_BAD_CLASS_ACCESS_FLAGS);
}
 ...
 *   returns:     nothing
 *   throws:      ClassFormatError if the field access flags are invalid
 *=======================================================================*/
...
verifyFieldFlags(unsigned short flags, unsigned short classFlags)
{
 ...
 failed:
    raiseExceptionCharMsg(ClassFormatError,KVM_MSG_BAD_FIELD_ACCESS_FLAGS);
}
 ...
 *   returns:     nothing
 *   throws:      ClassFormatError if signature is invalid
 *=======================================================================*/
...
verifyFieldType(const char* type)
{
    ...
    if (p == NULL || p - type != length) {
        raiseExceptionCharMsg(ClassFormatError,KVM_MSG_BAD_FIELD_SIGNATURE);
    }
}
                  ...
                  const char* name)
{
 ...
 failed:
    raiseExceptionCharMsg(ClassFormatError,KVM_MSG_BAD_METHOD_ACCESS_FLAGS);
}
...
verifyMethodType(const char *name, const char* signature)
{
    ...
    raiseExceptionCharMsg(ClassFormatError,KVM_MSG_BAD_METHOD_SIGNATURE);
 ...
 *   returns:     nothing
 *   throws:      ClassFormatError if the index is out of range or it
 *                indexes an entry not of the expected type
 *=======================================================================*/
                        ...
                        unsigned char tag)
{
    ...
    if (index >= length || tag2 != tag) {
        raiseExceptionCharMsg(ClassFormatError,KVM_MSG_BAD_CONSTANT_INDEX);
    }
}
...
static TRUSTEDCPENTRY
verifyTrustedCPEntry(POINTERLIST TrustedCP,
                        unsigned short index,
                        unsigned char tag)
{
    if (index >= TrustedCP->length || tag != TRUSTEDCP(index)->tag) {
        raiseExceptionCharMsg(ClassFormatError,SVM_MSG_BAD_CONSTANT_INDEX);
    }
    return TRUSTEDCP(index);
}

/*=========================================================================
 * FUNCTION:      verifyTrustedClassFlags()
 * TYPE:          private trusted class file load operation
 * OVERVIEW:      validate trusted class access flags
 * INTERFACE:
 *   parameters:  trusted class
 *   returns:     nothing
 *   throws:      ClassFormatError if the class access flags are invalid
 *=======================================================================*/

static void 
verifyTrustedClassFlags(TRUSTED_CLASS tclazz, unsigned short standardFlags)
{
    unsigned short flags = tclazz->accessFlags;

    /*
     * The class resource access key can only be non-null if privileged
     * class resource access is enforced.
     */
    if (tclazz->classResourceAccessKey != NULL &&
       (flags & TACC_CLASS_RESOURCE_ACCESS) != 0)
        goto failed;

    /*
     * The subclass key is NULL'ed if the class is final
     */
    if ((standardFlags & ACC_FINAL) != 0) {
       if (tclazz->subclassKey != NULL)
           tclazz->subclassKey = NULL;
    }
 ...
 failed:
    raiseExceptionCharMsg(ClassFormatError,
            SVM_MSG_BAD_TRUSTED_CLASS_ACCESS_FLAGS);
}
 ...
 *   returns:     <nothing>
 *   throws:      UnsupportedClassVersionError if version info is not as
 *                expected
 *                ClassFormatError if magic number is bad (i.e. not
 *                0xCAFEBABE)
 *=======================================================================*/
...
loadVersionInfo(FILEPOINTER_HANDLE ClassFileH)
{
    ...
    if (magic != 0xCAFEBABE) {
        raiseExceptionCharMsg(ClassFormatError,KVM_MSG_BAD_MAGIC_NUMBER);
    }
 ...
 *                the thisClass or superClass indexes are invalid or
 *                the superClass is invalid
 ...
 *   returns:     <nothing>
 *   throws:      ClassFormatError if any of the constant pool indexes in
 *                the interface table are invalid
 *=======================================================================*/
 ...
 *   parameters:  classfile pointer, current class pointer
 *   returns:     <nothing>
 *   throws:      ClassFormatError if any part of the fields table is
 *                invalid
 *=======================================================================*/
 ...
 *   throws:      ClassFormatError if any part of the exception_table is
 *                invalid
 *=======================================================================*/
 ...
 *   returns:     number of characters read from the class file
 *   throws:      ClassFormatError if any part of the stack maps are
 *                invalid
 *=======================================================================*/
    ...
    START_TEMPORARY_ROOTS
        ...
        for (stackMapIndex = 0; stackMapIndex < nStackMaps; stackMapIndex++) {
            ...
            for (index = 0, i = 0 ; i < 2; i++) {
                ...
                unsigned maxSize = (i == 0 ? thisMethod->frameSize 
 ...
 *   returns:     <nothing>
 *   throws:      ClassFormatError if any part of the Code or other method
 ...
 *   parameters:  classfile pointer, current class pointer
 *   returns:     <nothing>
 *   throws:      ClassFormatError if any part of the methods table is
 *                invalid
 *=======================================================================*/
    ...
    START_TEMPORARY_ROOTS
        ...
        if (!(thisMethod->accessFlags & (ACC_NATIVE | ACC_ABSTRACT))) {
            if ( thisMethod->frameSize < thisMethod->argCount) {
                raiseExceptionCharMsg(ClassFormatError,KVM_MSG_BAD_FRAME_SIZE);
            }
        }
...
/*=========================================================================
 * FUNCTION:      loadTrustedCP()
 * TYPE:          private class file load operation
 * OVERVIEW:      Load the constant pool in the Trusted attribute.
 * INTERFACE:
 *   parameters:  classfile pointer
 *   returns:     the loaded pool
 *   throws:      TrustedClassFormatError
 *=======================================================================*/

static POINTERLIST
loadTrustedCP(FILEPOINTER_HANDLE ClassFileH)
{
    unsigned int index;
    POINTERLIST result;
    /* we are reading in the following part of a TrustedAttribute:
     * 
    ...
    START_TEMPORARY_ROOTS
        DECLARE_TEMPORARY_ROOT(POINTERLIST, TrustedCP,
            (POINTERLIST)callocObject(SIZEOF_POINTERLIST(length),
                                      GCT_POINTERLIST));
        TrustedCP->length = length;
        /* the TrustedCP also starts at index one, just like the standard
         * ConstantPool.
         */
        for (index = 1; index != length; index++) {
            char tag = loadByte(ClassFileH);
            ...
            switch (tag) {
                ...
                case CONSTANT_DigitalSignature: {
                    /* Two 16-bit entries */
                    unsigned short length = loadShort(ClassFileH);
                    int size = SIZEOF_TRUSTEDCPENTRY(length);
                    TRUSTEDCPENTRY entry;

                    /* Allocate memory for entry and insert it into the pool */
                    entry = (TRUSTEDCPENTRY)mallocBytes(size);
                    TRUSTEDCP(index) = entry;
                    
                    /* Fill in fixed length fields */
                    entry->length = length;
                    loadBytes(ClassFileH,entry->data,length);

                    /* Verify CONSTANT_Utf8 entrys */
                    if (tag == CONSTANT_Utf8) {
                        verifyUTF8String((char*)entry->data, length);
                    }
                    break;
                }
                default: 
                    raiseExceptionCharMsg(TrustedClassFormatError,
                            SVM_MSG_INVALID_TRUSTEDCP_ENTRY);
                    break;
            }
            /* fill in the tag field */
            TRUSTEDCP(index)->tag = tag;
...
/*=========================================================================
 * FUNCTION:      loadNonDefaultFields()
 * TYPE:          private class file load operation
 * OVERVIEW:      Load the table of public & protected fields
 *                that don't use the default setting for accessibility with
 *                respect to U-classes.
 * INTERFACE:
 *   parameters:  classfile pointer
 *                pointer to class being loaded
 *                default setting
 *   returns:     <nothing>
 *   throws:      TrustedClassFormatError
 *=======================================================================*/
...
static void
loadNonDefaultFields(FILEPOINTER_HANDLE ClassFileH,
                       INSTANCE_CLASS CurrentClass,
                       bool_t defaultSetting)
{
    unsigned short count = loadShort(ClassFileH);
    unsigned short index;
    short nonDefaultFieldIndex = -1;
    FIELDTABLE table = CurrentClass->fieldTable;
     ...
     */
    if (count != 0 &&
        (((CurrentClass->clazz.accessFlags & ACC_PUBLIC) == 0) ||
        (table == NULL)))
        goto failed;
    /*
    ...
    for (index = 0; index != table->length; ++index) {
        FIELD field = &(table->fields[index]);
        if (count != 0) {
            unsigned short nextNonDefaultFieldIndex = loadShort(ClassFileH);
         ...
         */
        if ((field->accessFlags & (ACC_PROTECTED | ACC_PUBLIC)) == 0) {
            if (nonDefaultFieldIndex == index)
                goto failed;
            /*
             * This field is inaccessible from U-classes anyway but we
             * mark it as such for internal consistency.
             */
            field->uClassAccessible = 0;
            continue;
        }
        if (nonDefaultFieldIndex == index)
            field->uClassAccessible = !defaultSetting;
        else
            field->uClassAccessible = defaultSetting;
    }
    /*
    ...
    if (count == 0)
        return;
failed:
    raiseExceptionCharMsg(TrustedClassFormatError,SVM_MSG_INVALID_FIELD_SETTING);
}

/*=========================================================================
 * FUNCTION:      loadAndVerifyNonDefaultMethods()
 * TYPE:          private class file load operation
 * OVERVIEW:      Load the table of public & protected methods
 *                that don't use the default setting for accessibility with
 *                respect to U-classes.
 * INTERFACE:
 *   parameters:  classfile pointer
...
static void
loadNonDefaultMethods(FILEPOINTER_HANDLE ClassFileH,
                       INSTANCE_CLASS CurrentClass,
                       bool_t defaultSetting)
{
    unsigned short count = loadShort(ClassFileH);
    unsigned short index;
    short nonDefaultMethodIndex = -1;
    METHODTABLE table = CurrentClass->methodTable;
    ...
    for (index = 0; index != table->length; ++index) {
        ...
        if (count != 0) {
            unsigned short nextNonDefaultMethodIndex = loadShort(ClassFileH);
        ...
        /*
         * We are only interested in public or protected methods
         */
        if ((method->accessFlags & (ACC_PROTECTED | ACC_PUBLIC)) == 0) {
            if (nonDefaultMethodIndex == index)
                goto failed;
            /*
             * This method is inaccessible from U-classes anyway but we
             * mark it as such for internal consistency.
             */
            method->uClassAccessible = 0;
            continue;
        }
        if (nonDefaultMethodIndex == index)
            method->uClassAccessible = !defaultSetting;
        else
            method->uClassAccessible = defaultSetting;
    }
    /*
...
failed:
    raiseExceptionCharMsg(TrustedClassFormatError,SVM_MSG_INVALID_METHOD_SETTING);
}

/*=========================================================================
 * FUNCTION:      loadClassAttributes()
 * TYPE:          private class file load operation
 * OVERVIEW:      Load the "Trusted" attribute (if any) of a class,
 *                ignoring all the other possible class attributes.
 * INTERFACE:
 *   parameters:  classfile pointer
 *                runtime struct of class being loaded
 *                offset of ClassFile.attributes_count
 *                value of Trusted.length
 *   returns:     the number of bytes read
 *   throws:      TrustedClassFormatError
 *=======================================================================*/
#define CONSTANT_POOL_COUNT_OFFSET 8
#define TRUSTED_UTF8_ENTRY_SIZE (1 + 2 + 7)
 
#define ADJUST_SHORT_IN_BYTE_ARRAY(array,index,delta) \
    {\
        unsigned char* bytes = ((unsigned char*)array) + index; \
        unsigned short val = ((bytes[0]&0xFF) << 8) | (bytes[1]&0xFF); \
        val += delta; \
        bytes[0] = (val >> 8) & 0xFF; \
        bytes[1] = val & 0xFF; \
    }
...
static unsigned int 
loadTrustedAttribute(FILEPOINTER_HANDLE ClassFileH, 
                  INSTANCE_CLASS CurrentClass, 
                  unsigned int attrCountOffset)
{
    unsigned int attrOffset = getFilePosition(ClassFileH) - 6;
    TRUSTED_CLASS tclazz = CurrentClass->tclazz;
    unsigned short index;
    unsigned short subclassPermitsCount,
                   classResourceAccessPermitsCount,
                   refClassResourceAccessPermitsCount;
    unsigned int total;
    POINTERLIST TrustedCP;
   
    START_TEMPORARY_ROOTS
        unsigned int cpExtraEntryOffset;
        TRUSTEDCPENTRY entry;
        
        /* load constant_pool */
        IS_TEMPORARY_ROOT(TrustedCP,
                          loadTrustedCP(ClassFileH));

        /* load cp_extra_entry_offset */
        cpExtraEntryOffset = loadCell(ClassFileH);
      
        /* load csp_identifier */  
        index = loadShort(ClassFileH);
        verifyTrustedCPEntry(TrustedCP,index,CONSTANT_Utf8);
        if (!Crypto_VerifyCSP(TRUSTEDCP_STRING(index),
                    TRUSTEDCP(index)->length))
          raiseExceptionCharMsg(TrustedClassFormatError,SVM_MSG_INVALID_CSP);
      
        /* load trusted access control flags */
        tclazz->accessFlags = loadShort(ClassFileH) &
                              RECOGNIZED_TRUSTED_CLASS_FLAGS;

         /* load subclass_key */
        index = loadShort(ClassFileH);
        if (index != 0) {
            CryptoResultCode code;
            verifyTrustedCPEntry(TrustedCP,index,CONSTANT_PublicKey);
            entry = TRUSTEDCP(index);
            code = Crypto_DecodePublicKey(&(tclazz->subclassKey),
         ...
         /* load class_resource_key */
        index = loadShort(ClassFileH);
        if (index != 0) {
            CryptoResultCode code;
            verifyTrustedCPEntry(TrustedCP,index,CONSTANT_PublicKey);
            entry = TRUSTEDCP(index);
            code = Crypto_DecodePublicKey(&(tclazz->classResourceAccessKey),
                                       entry->length,entry->data);
            if (code != Crypto_OK)
                raiseExceptionCharMsg(TrustedClassFormatError,
                        CryptoResultCodeMessages[code]);
        ...
        /*
         * Now that the keys have been loaded, we can verify that they are
         * consistent with the access flags.
         */
        verifyTrustedClassFlags(tclazz,CurrentClass->clazz.accessFlags);
         ...
         */
        loadNonDefaultFields(ClassFileH, CurrentClass, loadByte(ClassFileH));
         ...
         * don't use this default setting.
         */
        loadNonDefaultMethods(ClassFileH, CurrentClass, loadByte(ClassFileH));
        
        /* load subclass_permits_count */
        subclassPermitsCount = loadShort(ClassFileH);

        /* load class_resource_access_permits_count */
        classResourceAccessPermitsCount = loadShort(ClassFileH);

        /* load ref_class_resource_access_permits_count */
        refClassResourceAccessPermitsCount = loadShort(ClassFileH);
        total = subclassPermitsCount + classResourceAccessPermitsCount +
                refClassResourceAccessPermitsCount;

        /*
         * Create the list of pending permits
         */
        if (total != 0) {
            /*
             * We allocate enough space for the permits
             */
            tclazz->ppermits = (PENDING_PERMITS)callocObject(
                    SIZEOF_PENDING_PERMITS(total),GCT_NOPOINTERS);
        }

        /* load subclass_permits */
        {
            unsigned short* ifaces = CurrentClass->ifaceTable;
            unsigned short ifaceCount = (ifaces == NULL ? 0 : ifaces[0]);

            for (index = 0; index != subclassPermitsCount; index++) {
                unsigned short interfaceIndex = loadShort(ClassFileH);
                unsigned short permitIndex = loadShort(ClassFileH);
                PERMIT permit;
                SIGNATURE signature;
                unsigned int pos = 0;
                CryptoResultCode code;
    
                bool_t isSubclassPermit = (index == 0 &&
                                           interfaceIndex == ifaceCount);
                /*
                 * If this is not the subclass permit, then this class must have
                 * one or more interfaces and the interfaceIndex must be a
                 * valid index into the ifaceTable of the class. If it is is
                 * valid, convert it to the constant pool index.
                 */
                if (!isSubclassPermit) {
                    if (ifaces == NULL || interfaceIndex >= ifaceCount)
                      raiseExceptionCharMsg(TrustedClassFormatError,
                              SVM_MSG_BAD_PERMIT);
                    /* First entry of the runtime interface table is its length */
                    interfaceIndex = ifaces[interfaceIndex + 1];
                }
                
                /*
                 * If this is not the subclass permit, allocate an internal
                 * permit and attach to permit list in class. This
                 * effectively makes it reachable to the garbage collector
                 * and so we don't have to make it a temporary root.
                 */
                if (!isSubclassPermit) {
                     permit = (PERMIT)callocObject(SIZEOF_PERMIT,GCT_NOPOINTERS);
                     pos = addPermit(tclazz->ppermits,permit,TRUE);
                }
    
                /*
                 * convert signature in Trusted constant pool to cryptographic
                 * provider specific runtime representation
                 */
                verifyTrustedCPEntry(TrustedCP,permitIndex,
                                      CONSTANT_DigitalSignature);
                entry = TRUSTEDCP(permitIndex);
                code = Crypto_DecodeSignature(&signature, entry->length,
                                           entry->data, tclazz);
                if (code != Crypto_OK)
                    raiseExceptionCharMsg(TrustedClassFormatError,
                            CryptoResultCodeMessages[code]);
                ...
                if (isSubclassPermit)
                    tclazz->ppermits->subclassPermit = signature;
                else {
                    permit = permitAt(tclazz->ppermits,pos);
                    permit->signature = signature;
                    verifyConstantPoolEntry(CurrentClass, interfaceIndex,
                                            CONSTANT_Class);
                    permit->grantor = (INSTANCE_CLASS)
                        CurrentClass->constPool->entries[interfaceIndex].clazz;
                    if (IS_ARRAY_CLASS(permit->grantor))
                        raiseExceptionCharMsg(TrustedClassFormatError,
                                SVM_MSG_BAD_PERMIT);
                }
            }
        ...
        /* load class_resource_access_permits */
        for (index = 0; index != classResourceAccessPermitsCount; index++) {
            unsigned short classIndex = loadShort(ClassFileH);
            unsigned short permitIndex = loadShort(ClassFileH);
            SIGNATURE signature;
            CryptoResultCode code;
            /*
             * allocate permit and attach to permit list in class. This
             * effectively makes it reachable to the garbage collector and so
             * we don't have to make it a temporary root.
             */
            PERMIT permit = (PERMIT)callocObject(SIZEOF_PERMIT,GCT_NOPOINTERS);
            unsigned int pos = addPermit(tclazz->ppermits,permit,FALSE);
            ...
            if (code != Crypto_OK)
                raiseExceptionCharMsg(TrustedClassFormatError,
                        CryptoResultCodeMessages[code]);
            /*
             * attach signature to permit
             */
            permit = permitAt(tclazz->ppermits,pos);
            permit->signature = signature;
            verifyConstantPoolEntry(CurrentClass, classIndex, CONSTANT_Class);
            permit->grantor = (INSTANCE_CLASS)
                CurrentClass->constPool->entries[classIndex].clazz;
            if (IS_ARRAY_CLASS(permit->grantor))
                raiseExceptionCharMsg(TrustedClassFormatError,
                        SVM_MSG_BAD_PERMIT);
            /*
             * Initialise the bits for the instantiatee's entry in the
             * constant pool that specify that the permit associated with the
             * class must be verified before a class resource can be accessed.
             */
            CONSTANTPOOL_TAG(CurrentClass->constPool,classIndex) |=
                CP_HAS_CLASS_RESOURCE_ACCESS_PERMIT;
            CONSTANTPOOL_TAG(CurrentClass->constPool,classIndex) &=
                ~CP_VERIFIED_CLASS_RESOURCE_ACCESS;
        }

        /* load ref_class_resource_access_permits */
        for (index = 0; index != refClassResourceAccessPermitsCount; index++) {
            unsigned short classIndex = loadShort(ClassFileH);
            unsigned short permitIndex = loadShort(ClassFileH);
            SIGNATURE signature;
            CryptoResultCode code;
            ...
            permit = permitAt(tclazz->ppermits,pos);
            permit->signature = signature;
            verifyTrustedCPEntry(TrustedCP,classIndex,CONSTANT_Utf8);
            START_TEMPORARY_ROOTS
                DECLARE_TEMPORARY_ROOT(const char *, name, 
                    TRUSTEDCP_STRING(classIndex));
                    verifyName(name, LegalClass);
                    permit->grantor = (INSTANCE_CLASS)
                        getRawClassX(&name, 0, TRUSTEDCP(classIndex)->length);
            END_TEMPORARY_ROOTS
            if (IS_ARRAY_CLASS(permit->grantor))
                raiseExceptionCharMsg(TrustedClassFormatError,
                        SVM_MSG_BAD_PERMIT);
         ...
         * We need to
         * reconstruct the contents that were signed to produce all the
         * permits attached to this class. For the subclassing and
         * class resource permits, the content is the original pre-wobulated
         * class file. For the domain permits, the content is the original
         * pre-wobulated class file appended with the contents of the Trusted
         * attribute sans the last 2 items (i.e. the domain items).
         */
        {
            unsigned int currentOffset = getFilePosition(ClassFileH);
            unsigned int classSize     = attrOffset -
                                         (cpExtraEntryOffset ?
                                          TRUSTED_UTF8_ENTRY_SIZE : 0);
            unsigned int trustedSize   = (currentOffset - (attrOffset+6));
            DECLARE_TEMPORARY_ROOT(char*,contents,
                                   mallocBytes(classSize + trustedSize));
            unsigned char* buf = contents;
            DIGEST digest;
            SIGNATURE signature1 = NULL;
                                   
            /* rewind to the beginning of the class file */
            setFilePosition(ClassFileH,0);
            if (cpExtraEntryOffset != 0) {
              /* read up to "Trusted" Utf8 entry and skip it */
              unsigned int toRead = cpExtraEntryOffset;
              loadBytes(ClassFileH,buf,toRead);
              skipBytes(ClassFileH,TRUSTED_UTF8_ENTRY_SIZE);
              buf += toRead;
              
              /* read up to the Trusted attribute and skip the first 6 bytes
               * of it (tag + length)
               */
              toRead = attrOffset-(cpExtraEntryOffset+TRUSTED_UTF8_ENTRY_SIZE);
              loadBytes(ClassFileH,buf,toRead);
              skipBytes(ClassFileH,6);
              buf += toRead;
        
              /* read up to the Trusted.domains_count item */
              toRead = currentOffset - (attrOffset + 6);
              loadBytes(ClassFileH,buf,toRead);
              
              /* patch up the ClassFile.constant_pool_count value */
              ADJUST_SHORT_IN_BYTE_ARRAY(contents,CONSTANT_POOL_COUNT_OFFSET,-1);
              attrCountOffset -= TRUSTED_UTF8_ENTRY_SIZE;
            }
            else {
              /* read up to the Trusted attribute and skip the first 6 bytes
               * of it (tag + length)
               */
              unsigned int toRead = attrOffset;
              loadBytes(ClassFileH,buf,toRead);
              skipBytes(ClassFileH,6);
              buf += toRead;
            ...
            /* patch up the ClassFile.attributes_count value */
            ADJUST_SHORT_IN_BYTE_ARRAY(contents,attrCountOffset,-1);

            /*
             * Compute the digest of the contents that are signed by the
             * subclass and class resource permits (if any)
             */
            if (tclazz->ppermits != NULL)
            {
                Crypto_DigestContent(&digest, classSize,contents);
                /*
                 * attach digest to PENDING_PERMITS
                 */
                tclazz->ppermits->digest = digest;
            }
            
            /*
             * Compute the digest of the contents that were signed to
             * generate the domain signature(s).
             */
            Crypto_DigestContent(&digest,classSize+trustedSize,contents);
            IS_TEMPORARY_ROOT(digest,digest);
            
            /*
             * Now read in the domain keys and signatures.
             */
            total = loadShort(ClassFileH);
            if (total == 0)
                /*
                 * Every trusted class must at least have a primary trusted
                 * domain.
                 */
                raiseExceptionCharMsg(TrustedClassFormatError,
                        SVM_MSG_INVALID_DOMAINS_COUNT);
            tclazz->domainsLength = total;
            tclazz->domainKeys = (KEY*)callocPermanentObject(
                    ByteSizeToCellSize(sizeof(KEY*) * total));

            for (index = 0; index != total; index++) {
                /* load and decode domain key */
                unsigned short length = loadShort(ClassFileH);
                CryptoResultCode code;
                buf = mallocBytes(length);
                loadBytes(ClassFileH,buf,length);
                code = Crypto_DecodePublicKey(&(tclazz->domainKeys[index]),
                                       length,buf);
                if (code != Crypto_OK)
                    raiseExceptionCharMsg(TrustedClassFormatError,
                            CryptoResultCodeMessages[code]);
                ...
                /*
                 * Ensure that the domain key is unique.
                 */
                for (length = 0; length < index; length++)
                    if (tclazz->domainKeys[length] == tclazz->domainKeys[index])
                        raiseExceptionCharMsg(TrustedClassFormatError,
                                SVM_MSG_DUPLICATE_DOMAIN_KEYS);
                 
                /* load, decode and verify domain signature */
                length = loadShort(ClassFileH);
                buf = mallocBytes(length);
                loadBytes(ClassFileH,buf,length);
                if ((code = Crypto_DecodeSignature(&signature1,length,buf,NULL))
                         != Crypto_OK)
                    raiseExceptionCharMsg(TrustedClassFormatError,
                            CryptoResultCodeMessages[code]);
                if ((code = Crypto_VerifySignature(signature1,digest,
                            tclazz->domainKeys[index])) != Crypto_OK)
                    raiseExceptionCharMsg(IllegalDomainError,
                            CryptoResultCodeMessages[code]);
                    
            }
        }
    END_TEMPORARY_ROOTS
    return (getFilePosition(ClassFileH) - (attrOffset + 6));
}
  
static void 
loadClassAttributes(FILEPOINTER_HANDLE ClassFileH, 
                    INSTANCE_CLASS CurrentClass, 
                    POINTERLIST_HANDLE StringPoolH)
{
    unsigned int attrCountPos = getFilePosition(ClassFileH);
    unsigned short attrCount = loadShort(ClassFileH);
    int attrIndex;
    bool_t haveTrusted = FALSE;

    /*  See if the class has any attributes in the class file */
    for (attrIndex = 0; attrIndex < attrCount; attrIndex++) {
        unsigned short attrNameIndex = loadShort(ClassFileH);
        unsigned int   attrLength    = loadCell(ClassFileH);
        char*          attrName      = getUTF8String(StringPoolH, attrNameIndex);
        /*  Check if the attribute contains source code */
        if (strcmp(attrName, "Trusted") == 0) {
            int actualLength;

            if (haveTrusted)
                /* there can only be one Trusted attribute */
                raiseExceptionCharMsg(TrustedClassFormatError,
                        SVM_MSG_DUPLICATE_TRUSTED_ATTRIBUTE);
            else if (attrIndex != attrCount - 1)
                /* the Trusted attribute must be the last class attribute */
                raiseExceptionCharMsg(TrustedClassFormatError,
                        SVM_MSG_TRUSTED_NOT_LAST);
            else
                haveTrusted = TRUE;
            
            /* allocate memory for the extra trusted class info */
            CurrentClass->tclazz =
                (TRUSTED_CLASS)callocPermanentObject(SIZEOF_TRUSTED_CLASS);
            actualLength = loadTrustedAttribute(ClassFileH,
                                                   CurrentClass,
                                                   attrCountPos);
            if (actualLength != attrLength) { 
                raiseExceptionCharMsg(TrustedClassFormatError,
                        SVM_MSG_BAD_TRUSTED_ATTRIBUTE_LENGTH);
            }

            
        } else {
            /*  Unrecognized or ignored attribute (e.g "SourceFile" or
             *  "Deprecated"); skip */
            skipBytes(ClassFileH, attrLength);
        }
    }
 ...
 *   parameters:  classfile pointer, current class pointer
 *   returns:     <nothing>
 *   throws:      ClassFormatError if an attribute's name is invalid
 *=======================================================================*/
...
static void loadRawClass(INSTANCE_CLASS clazz)
{
        START_TEMPORARY_ROOTS
            ...
            if (!ClassFile) {
                clazz->status = CLASS_RAW;
                if (LoadingViaReflection) {
                    LoadingViaReflection = FALSE;
                    raiseException("java/lang/ClassNotFoundException");
                }                    
                else {
                    raiseExceptionCharMsg(NoClassDefFoundError,
                        getClassName((CLASS)clazz));
                }
            }
            /*
             * Only the "initiating class" (i.e. the one being loaded via
             * Class.forName) should cause a different exception to be
             * thrown.
             */
            LoadingViaReflection = FALSE;
            ...
            if (traceclassloading || traceclassloadingverbose) {
                Log->loadClass(getClassName((CLASS)clazz));
            }
#endif
            ...
            IS_TEMPORARY_ROOT(StringPool, 
                              loadConstantPool(&ClassFile, clazz));

            /*  Load class identification information */
            loadClassInfo(&ClassFile, clazz);

            /*  Load interface pointers */
            loadInterfaces(&ClassFile, clazz);

            /*  Load field information */
            loadFields(&ClassFile, clazz, &StringPool);

            /*  Load method information */
            loadMethods(&ClassFile, clazz, &StringPool);
#if SVM
            /*  Load the Trusted attribute if there is one */
            loadClassAttributes(&ClassFile, clazz, &StringPool);
#else
...
/*
 * Both an iterative and recursive implementation of the class loader are
 * included below. The recursive version is obviously simpler
 * conceptually but not as efficient on stack resources as the iterative
 * implementation. The iterative implementation is still recursive when
 * traversing an interface class hierachy (as was the original
 * implementation).
 */
#define ITERATIVE_CLASS_LOADER 1
#if ITERATIVE_CLASS_LOADER
...
void
loadClassfile(INSTANCE_CLASS InitiatingClass)
{
    /*
    ...
    if (clazz->status != CLASS_RAW)
        fatalVMError("Expected class status of CLASS_RAW");
    ...
    TRY {
        ...
        while ((clazz = findSuperMostUnlinked(InitiatingClass)) != NULL) {
            ...
            if (clazz->superClass == NULL) {
                ...
                if (superClass->clazz.accessFlags & ACC_FINAL) {
                    raiseExceptionCharMsg(VerifyError,
            ...
            if (clazz->ifaceTable != NULL) {
                int ifIndex;
                for (ifIndex = 1; ifIndex <= clazz->ifaceTable[0]; ifIndex++) {
                     ...
                     */
                        ...
                        loadClassfile(ifaceClass);
                    }
                    else if (ifaceClass->status < CLASS_LINKED) {
                        fatalVMError("Expected class status >= CLASS_LINKED");
                    }
...
#if SVM
            /*
             * Now that the subclass and all interfaces (if any) are guaranteed
             * to be in the system, we can verify all privileges
             * supplied/required for them.
             */
            if (clazz != JavaLangObject)
                verifySubclassPrivileges(clazz);
#endif /* SVM */
         ...
         */
        sprintf(str_buffer,"While loading %s%s",
                getClassName((CLASS)InitiatingClass),
                    (e->message == NULL ? "" : ": "));
        if (e->message != NULL) {
            int length = strlen(str_buffer);
            getStringContentsSafely(e->message,str_buffer+length,
 ...
 *   parameters:  class structure.  Only the name field is filled in.  All
 *                other fields must be NULL and the class status must be
 *                CLASS_RAW or CLASS_LOADING
 *   returns:     pointer to runtime class structure or NIL if not found
 *   throws:      LinkageError
 *=======================================================================*/

void
loadClassfile(INSTANCE_CLASS clazz)
{
    static int ClassesBeingLoaded = 0;

    /*
     * This keeps track of whether or not this is a recursive call to this
     * function.
     */
    ClassesBeingLoaded = 1;

    TRY {
        /*
         * Ensure that clazz is not already being loaded. If it is, then this
         * is a ClassCircularityError.
         */
        if (clazz->status != CLASS_RAW) {
            if (clazz->status != CLASS_LOADING)
                fatalVMError("Expected class status of CLASS_LOADING");
            raiseException(ClassCircularityError);
        }

        /*
         * Load the raw class binary.
         */
        clazz->status = CLASS_LOADING;
        loadRawClass(clazz);
        clazz->status = CLASS_LOADED;
        ...
        /*
         * Load superclass(es) and ensure the subclass<->superclass
        ...
        if (clazz->superClass == NULL) {
             ...
             */
            if (clazz != JavaLangObject) {
                raiseExceptionCharMsg(ClassFormatError,KVM_MSG_BAD_SUPERCLASS);
            }
            /*
             * Treat 'java.lang.Object' specially (it has no superclass
             */
            ...
            /*
             * Ensure that the superclass did not fail to initialize at
             * some earlier point.
             */
            if (superClass->status == CLASS_ERROR) {
                raiseExceptionCharMsg(NoClassDefFoundError,
                        getClassName((CLASS)superClass));
            }
            /*
             * Recursively load the superclass if necessary
             */
            if (superClass->status == CLASS_RAW) {
                loadClassfile(superClass);
            }
             ...
             */
            if (superClass != JavaLangObject &&
                (superClass->clazz.accessFlags & ACC_INTERFACE)) {
                raiseExceptionCharMsg(ClassFormatError,KVM_MSG_BAD_SUPERCLASS);
            }
        ...
        /*
         * Load super-interface(s) and ensure the subclass<->interface
         * relationship is valid.
         */
        if (clazz->ifaceTable != NULL) {
            int ifIndex;
            for (ifIndex = 1; ifIndex <= clazz->ifaceTable[0]; ifIndex++) {
                INSTANCE_CLASS ifaceClass = (INSTANCE_CLASS)
                    clazz->constPool->entries[clazz->ifaceTable[ifIndex]].clazz;
                ...
                /*
                 * Ensure that the superinterace did not fail to initialize at
                 * some earlier point.
                 */
                if (ifaceClass->status == CLASS_ERROR) {
                    raiseExceptionCharMsg(NoClassDefFoundError,
                ...
                /*
                 * Resolve the interface class. This will detect any circularity
                 * errors.
                 */
                }
                if (ifaceClass->status == CLASS_RAW) {
                    loadClassfile(ifaceClass);
                }
            
        
                /*  JVM Spec 5.3.5 */
                if ((((CLASS)ifaceClass)->accessFlags & ACC_INTERFACE) == 0) {
                    raiseExceptionCharMsg(IncompatibleClassChangeError,
                        KVM_MSG_CLASS_IMPLEMENTS_NON_INTERFACE);
                }
         ...
         * of the exception if this is the initiating class.
         */
        if (ClassesBeingLoaded == 0) {
            sprintf(str_buffer,"While loading %s%s",getClassName((CLASS)clazz),
                    (e->message == NULL ? "" : ": "));
            if (e->message != NULL) {
                int length = strlen(str_buffer);
                getStringContentsSafely(e->message,str_buffer+length,
            ...
            e->message =
                instantiateString(str_buffer,strlen(str_buffer));
        }
        /*
         * Errors occuring during classfile loading are "transient" errors.
         * That is, their cause is temporal in nature and may not occur
         * if a different classfile is submitted at a later point.
         */
        revertToRawClass(clazz);
        THROW(e);
    } END_CATCH
}

#endif /* ITERATIVE_CLASS_LOADER */
