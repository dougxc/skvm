...
#if SVM
                if (iclazz->tclazz) {
                    TRUSTED_CLASS tclazz = iclazz->tclazz;
                    if (tclazz->ppermits != NULL) {
                        unsigned int i;
                        MARK_OBJECT(tclazz->ppermits);
                        /*
                         * mark all the SIGNATUREs in the non-null PERMITS
                         */
                        for (i = 0; i != tclazz->ppermits->count; i++)
                        {
                            PERMIT permit = tclazz->ppermits->permits[i];
                            MARK_OBJECT(permit);
                            MARK_OBJECT_IF_NON_NULL(permit->signature);
                        }
                        /*
                         * mark the associated DIGEST
                         */
                        MARK_OBJECT_IF_NON_NULL(tclazz->ppermits->digest);
                        /*
                         * mark the subclass permit.
                         */
                        MARK_OBJECT_IF_NON_NULL(tclazz->ppermits->subclassPermit);
                     }
                }
#endif /* SVM */
        ...
        FOR_ALL_CLASSES(clazz)
            ...
            if (!IS_ARRAY_CLASS(clazz)) {
                ...
                if (iclazz->tclazz) {
                    ...
                    if (tclazz->ppermits != NULL) {
                        unsigned int i;
                        updatePointer(&tclazz->ppermits, currentTable);
                        /*
                         * update all the non-null PERMITs and their non-null
                         * SIGNATUREs
                         */
                        for (i = 0; i!=tclazz->ppermits->count; i++)
                        {
                            updatePointer(&tclazz->ppermits->permits[i],
                                          currentTable);
                            updatePointer(&tclazz->ppermits->permits[i]->
                                          signature, currentTable);
                        }
                        /*
                         * update the associated DIGEST
                         */
                        updatePointer(&tclazz->ppermits->digest, currentTable);
                        /*
                         * update the subclass permit.
                         */
                        updatePointer(&tclazz->ppermits->subclassPermit,currentTable);
                     }
                }
#endif /* SVM */
...
markThreadStack(THREAD thisThread)
{
    ...
    while (thisFP != NULL) {
        ...
        } else if (method == InitClassMethod &&
                    method->ofClass->status < CLASS_VERIFIED) {
            totalSize = 1;
            map[0] = 0xFF;
#endif /* CLASS_INITIALIZATION_IN_JAVA */
        } else {
            unsigned int expectedStackSize;
            expectedStackSize = getGCRegisterMask(method, thisIP, map);
            if (INCLUDEDEBUGCODE && (realStackSize > expectedStackSize)) {
                fatalError(KVM_MSG_CONFLICTING_STACK_SIZES);
            }
        }
    ...
    for (;;) {
        ...
        if (method == RunCustomCodeMethod) {
            memset(map, -1, (realStackSize + 7) >> 3);
                    ...
                    method->ofClass->status < CLASS_VERIFIED) {
            /*
             * The java.lang.Class has not yet been verified
             * and so its stack map has not yet been converted into the
             * pointermap required by the GC. As this class must be the
             * first to be initialized, we know that the method is not yet
             * executing and so we also therefore know exactly what the
             * stackmap is like at offset 0 (i.e. only the 'this' pointer
             * of the class being initialized is in the first local
             * variable slot).
             */
            totalSize = 1;
            map[0] = 0xFF;
