...
#if STANDARDBYTECODES
SELECT(ICONST_M1)       /* Push null reference onto the operand stack */
        pushStack(-1);
DONE(1)
#endif
...
#if STANDARDBYTECODES
SELECT(ILOAD_3)         /*  Load integer from forth local variable */
        pushStack(lp[3]);
DONE(1)
#endif
...
#if STANDARDBYTECODES
SELECT(LLOAD_3)         /*  Load integer from forth local variable */
        pushStack(lp[3]);
        pushStack(lp[4]);
DONE(1)
#endif
...
#if FLOATBYTECODES
SELECT(FLOAD_3)         /*  Load integer from forth local variable */
        pushStack(lp[3]);
DONE(1)
#endif
...
#if FLOATBYTECODES
SELECT(DLOAD_3)         /*  Load integer from forth local variable */
        pushStack(lp[3]);
        pushStack(lp[4]);
DONE(1)
#endif
...
#if STANDARDBYTECODES
SELECT(ALOAD_3)         /*  Load integer from forth local variable */
        pushStack(lp[3]);
DONE(1)
#endif
...
#if STANDARDBYTECODES
SELECT(ISTORE_3)        /*  Store integer into forth local variable */
        lp[3] = popStack();
DONE(1)
#endif
...
#if STANDARDBYTECODES
SELECT(LSTORE_3)        /*  Store long into forth local variable */
        lp[4] = popStack();
        lp[3] = popStack();
DONE(1)
#endif
...
#if FLOATBYTECODES
SELECT(FSTORE_3)        /*  Store float into forth local variable */
        lp[3] = popStack();
DONE(1)
#endif
...
#if FLOATBYTECODES
SELECT(DSTORE_3)        /*  Store double into forth local variable */
        lp[4] = popStack();
        lp[3] = popStack();
DONE(1)
#endif
...
#if STANDARDBYTECODES
SELECT(ASTORE_3)        /*  Store address into forth local variable */
        lp[3] = popStack();
DONE(1)
#endif
...
SELECT(INVOKEVIRTUAL)
        ...
        if (cpMethod) {
            ...
            VMSAVE
            thisMethod = lookupMethod(dynamicClass, cpMethod->nameTypeKey,
                                      fp_global->thisMethod->ofClass);
            VMRESTORE
            if (thisMethod) {
...
SELECT(NEW)                             /*  Create new object */
        ...
        */
        ...
        if (thisClass->clazz.accessFlags & (ACC_INTERFACE | ACC_ABSTRACT)) {
            raiseException(InstantiationError);
...
SELECT(ATHROW)                          /* Throw exception or error */
        ...
        VMSAVE
            START_TEMPORARY_ROOTS
                DECLARE_TEMPORARY_ROOT(THROWABLE_INSTANCE, exceptionX,
                                       exception);
#if PRINT_BACKTRACE
                if (exceptionX->backtrace == NULL) {
                    fillInStackTrace(&exceptionX);
                }
#endif
                throwException(&exceptionX);
            END_TEMPORARY_ROOTS
        VMRESTORE
DONE_R
#endif
