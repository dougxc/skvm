...
bool_t LoadingViaReflection = FALSE;
void Java_java_lang_Class_forName(void)
{
    /*
     * It is very important that this function always keeps the stack
     * pointer for the frame of the caller exactly where it was upon entry
     * to this function. This ensures that the GC will correctly use the
     * stackmap of the caller when collecting.
     */
            ...
            if (isValidName(className, LegalClass)) {
                ...
                if (!IS_ARRAY_CLASS(thisClass)) {
                    ...
                    if (iclass->status == CLASS_RAW) {
                        /*
                         * The following flag tells the loader to throw
                         * a ClassNotFoundException only the very first
                         * classfile it tries to receive cannot be found.
                         * Any other error during the loading will result
                         * in the appropriate LinkageError subclass being
                         * thrown.
                         */
                        LoadingViaReflection = TRUE;
                        loadClassfile(iclass);
                    }
...
void Java_java_lang_Class_newInstance(void)
{
    ...
    CLASS clazz = popStackAsType(CLASS);
    ...
    if (classHasAccessToClass(currentClass, clazz)) {
        ...
        if (   (method != NULL)
            && (method->ofClass == (INSTANCE_CLASS)clazz)
                ...
                * cuurentClass is a subclass of clazz). Protected constructors
...
#if SVM
                   /*
                    * These extra parameters are used to test whether or
                    * not we have access to the constructors of the
                    * class we are about to instantiate. That is, whether
                    * or not we have permission to instantiate the class.
                    */
                   (method->uClassAccessible),
                   TRUE,
                   NULL,
                   0,
#endif /* SVM */
            ...
            )
        {
            START_TEMPORARY_ROOTS
                ...
                if (object != NULL) {
                    ...
                    /* We now need to call the default constructor.
                     * We'd like to just
                     ...
                     * method that called newInstance.
                     */
                    ...
                    /*
                     * Now push the argument for the default constructor
...
void Java_java_lang_System_getProperty0(void)
{
    ...
    result = value == NULL ? NULL : instantiateString(value, strlen(value));
    topStackAsType(STRING_INSTANCE) = result;
}
...
static void string_indexOf(int fromIndex) {
    long ch = popStack();
    STRING_INSTANCE this = popStackAsType(STRING_INSTANCE);
...
void Java_java_lang_StringBuffer_append__Ljava_lang_String_2(void) {
    ...
    long count, newCount, stringLength;
