...
#if SVM
    TRUSTED_CLASS tclazz;           /* Extra info for trusted classes */
#endif /* SVM */
};
...
/* TRUSTED_CLASS */
struct trustedClassStruct {

  /*
   * Trusted access modifiers (i.e. TACC_SUBCLASS, TACC_CLASS_RESOURCE and
   * TACC_EXCEPTION).
   */
  unsigned short accessFlags;
  /*
   * The key used to verify subclassing privileges granted by this class.
   */
  KEY subclassKey;
  /*
   * The key used to verify class resource access privileges granted by
   * this class.
   */
  KEY classResourceAccessKey;
  /*
   * The security domains. The domains array is a permanent
   * object and thus need not concern the garbage collector.
   */
  unsigned short domainsLength;
  KEY* domainKeys;
  
  PENDING_PERMITS ppermits;
};

#define IS_TCLASS(c)     ((c)->tclazz != NULL)
#define IS_TINSTANCE(i)  ((!IS_ARRAY_CLASS((i)->clazz)) && \
                          IS_TCLASS((INSTANCE_CLASS)((i)->clazz)))
...
#if SVM
#define SIZEOF_TRUSTED_CLASS      StructSizeInCells(trustedClassStruct)
#endif
...
/*
 * A global flag used to communicate to the loader that it has been invoked
 * as a result of a call to Class.forName. The loader can then throw the
 * appropriate exception if the load fails.
 */
extern bool_t LoadingViaReflection;

#if SVM
/*=========================================================================
 * Operations on trusted classes
 *=======================================================================*/

bool_t classDomainsIntersect(TRUSTED_CLASS clazz1, TRUSTED_CLASS clazz2);
bool_t classPrimaryDomainMatch(TRUSTED_CLASS clazz1, TRUSTED_CLASS clazz2);

void verifySubclassPrivileges(INSTANCE_CLASS clazz);
void verifyClassResourceAccessPrivilege(INSTANCE_CLASS grantor,
                                  unsigned int cpIndex,
                                  INSTANCE_CLASS requestor);
void verifyClassResourceAccessPrivilege_Reflection(INSTANCE_CLASS grantor,
                                             INSTANCE_CLASS requestor);
