 ...
 * These values flags are used to encode extra info about field types and
 * method return types. */
...
#if SVM
/*
 * The implementation of class resource access privilege verification iis
 * implemented as follows. We use 2 of the 3 unused bits in the tag for a
 * constant pool entry to store information about the 'class resource
 * accessability' of
 * a CONSTANT_Class entry. The CP_CLASS_RESOURCE_PERMIT flag is set at load
 * time if the class has a class resource access permit for the indexed class.
 * The CP_VERIFIED_CLASS_RESOURCE_PERMIT flag is set once the indexed class
 * has been
 * resolved and had the class resource access privilege verified. Subsequent
 * accesses to class resources need only check these 2 bits to determine
 * whether a permit is available and/or needs to be verified.
 *
 *
 * CP_VERIFIED_CLASS_RESOURCE -----------+
 * CP_HAS_CLASS_RESOURCE_PERMIT--------+ |       
 *                 CP_CACHEBIT-------+ | |
 *                                   v v v   <-tag->
 *                                  |-+-+-+-+-+-+-+-|
 *                                  | | | | | | | | | 
 ...
 * 
 */
#define CP_HAS_CLASS_RESOURCE_ACCESS_PERMIT 0x40
#define CP_VERIFIED_CLASS_RESOURCE_ACCESS   0x20

/*
 * We need to redefine the mask used to extract the CONSTANT_* value from the
 * tag byte.
 */
#undef CP_CACHEMASK
#define CP_CACHEMASK 0x1F
...
#if SVM
/*=========================================================================
 * Security access flags from SVM Specification
 *=======================================================================*/


/* These flags are used to indiciate how a trusted class' behaviour is
 * modified with respect to standard Java semantics.
 */
#define TACC_SUBCLASS              0x0001
#define TACC_CLASS_RESOURCE_ACCESS 0x0002
#define TACC_EXCEPTION             0x0004

/* These are the tag values for entries in the Trusted constant pool */
#define CONSTANT_PublicKey        13
#define CONSTANT_DigitalSignature 14
...
/* In contrast to the ClassFile.constant_pool, the constant_pool found in
 * a Trusted attribute of a trusted classfile is a load-time only
 * construct. In addition, each of its entries is exactly the same in
 * structure. As such, we opt for a simpler representation that simply mimics
 * it's classfile layout.
 */

/* TRUSTEDCPENTRY */
struct trustedCPEntryStruct {
    unsigned char tag;      /* type tag */
    unsigned short length;  /* size of data */
    char data[1];           /* data (e.g. key/signature/string) */
};

/* Macro to calculate the size of a Trusted constant pool entry based on the
 * length of the data (in bytes)
 */ 
#define SIZEOF_TRUSTEDCPENTRY(n) \
    (offsetof(struct trustedCPEntryStruct, data) + n)
