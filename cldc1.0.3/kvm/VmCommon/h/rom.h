...
{ { &AllClassblocks.java_lang_Class, { NULL } , \
    ...
    (METHODTABLE)methods, (unsigned short*)intfs, NULL /* statics */, size, status, NULL SVM_TCLAZZ }
...
#if SVM
#define SVM_METHOD_FLAGS(flags) flags, 0
#else
#define SVM_METHOD_FLAGS(flags) flags
#endif /* SVM */
...
#   define METHOD_INFO(class, code, handlers, stackMaps, flags, argSize, frameSize, maxStackSize, codeSize, nameTypeKey) \
        ...
        SVM_METHOD_FLAGS(flags), &AllClassblocks . class, \
        frameSize, argSize }
...
#   define ABSTRACT_METHOD_INFO(class, flags, argSize, nameTypeKey)     \
        { nameTypeKey,                                                  \
        { { NULL, NULL, {NULL}, 0} },                                   \
        SVM_METHOD_FLAGS(flags), &AllClassblocks . class, 0, argSize }
...
#if SVM
#define SVM_FIELD_FLAGS(flags) flags, 0
#else
#define SVM_FIELD_FLAGS(flags) flags
#endif /* SVM */

#if __GNUC__
#   define FIELD_INFO(class, flags, off, nameTypeKey) \
       { nameTypeKey, SVM_FIELD_FLAGS(flags), &AllClassblocks . class, { offset: off} }
#    define STATIC_FIELD_INFO(class, flags, off, nameTypeKey) \
       { nameTypeKey, SVM_FIELD_FLAGS(flags), &AllClassblocks . class, { staticAddress: (KVM_staticData + off) } }
#else
#   define FIELD_INFO(class, flags, offset, nameTypeKey) \
       { nameTypeKey, SVM_FIELD_FLAGS(flags), &AllClassblocks . class, { offset } }
#    define STATIC_FIELD_INFO(class, flags, offset, nameTypeKey) \
       { nameTypeKey, SVM_FIELD_FLAGS(flags), &AllClassblocks . class, { (long)(KVM_staticData + offset) } }
#endif /* __GNUC__ */
