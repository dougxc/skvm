...
WndProc (HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CHAR:
        StoreKVMEvent(keyDownKVMEvent, 1, (cell)wParam);
        return 0;

    case WM_KEYDOWN:
        if (wParam >= 37 && wParam <= 40) {
            StoreKVMEvent(keyDownKVMEvent, 1, (cell)ArrowKeyMap[wParam - 37]);
            return 0;
        }
        break;
...
void
winGetAndStoreNextKVMEvent(int forever)
{
    MSG msg ;
