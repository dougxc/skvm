...
bool_t
inflate(void *compData, /* compressed data source */
        JarGetByteFunctionType getByte,
...
inflateStored(inflaterState *state)
{
    ...
    if (len + nlen != 0xFFFF) {
        ...
        int lastOffset = outOffset + len;
        for ( ; outOffset < lastOffset; outOffset++) {
            unsigned char value = NEXTBYTE;
