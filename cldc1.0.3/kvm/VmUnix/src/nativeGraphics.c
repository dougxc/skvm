...
/* Constants used in the drawing functions. Should correlate */
/* with those defined in com.sun.kjava.Graphics.java. */
#define MODE_PLAIN  1
#define MODE_GRAY   2
#define MODE_ERASE  3
...
void Java_com_sun_kjava_Graphics_drawRectangle() {
    ...
    if (width <= 0 || height <= 0) { 
        ...
        if (arcHeight == height && arcWidth == width) { 
            ...
            rect[1].width = halfAW;
            rect[1].y = y + halfAH;
            rect[1].height = height - arcHeight;

            /* The notch on the right */
            rect[2].x = x + width - halfAW;
            rect[2].width = halfAW;
            rect[2].y = y + halfAH;
            rect[2].height = height - arcHeight;
...
void Java_com_sun_kjava_Graphics_drawBitmap()
{
    ...
    p = pixmapFromPilotData(data, width, height, rowBytes);
...
pixmapFromPilotData(short *data, int width, int height, int rowBytes)
{
    ...
    GC gc = XCreateGC(display, p, (unsigned long) 0, (XGCValues *) 0);
        ...
        unsigned char swap; 
        int max, j;
        short *ndata = (short *) malloc(rowBytes * height);
        
        if (ndata == NULL) 
        {
            fprintf(stderr, "Memory allocation for graphics failed\n");
            exit(1);
        }
        max = (rowBytes * height) / 2;
        for (j = 0; j < max; j++)
        {
            swap = data[j] & 0xff;
            ndata[j] = ((data[j] >> 8) & 0xff) | ((swap << 8) & 0xff00);
        }
        data = ndata;
    #endif    
    ...
    #else
    {
        ...
        visualInfo = XGetVisualInfo (display, VisualNoMask, 0, &items);
        if (items < 1)
        {
            fprintf(stderr, "Could not get visual info\n");
            exit(1);
        }
        ximage = XCreateImage (display, visualInfo->visual, 1, ZPixmap, 0,
                               (char*)data, width, height, 16, rowBytes);

        if (ximage->bitmap_bit_order != MSBFirst)
            for (i = 0; i < height * rowBytes; i++)
            {
                byte    = ((unsigned char*)data)[i];
                ((unsigned char*)data)[i] = ((byte &   1) << 7)
                                          | ((byte &   2) << 5)
                                          | ((byte &   4) << 3)
                                          | ((byte &   8) << 1)
                                          | ((byte &  16) >> 1)
...
GetAndStoreNextKVMEvent(bool_t forever, ulong64 waitUntil) 
{
    ...
    if (!windowSystemInitialized) {
        if (forever || !ll_zero_eq(waitUntil)) { 
            InitializeWindowSystem(); 
            display = kauaiDisplay;
        } else { 
            /* Don't start the window system unless we were supposed to
             * wait!
             */
            return;
        }
    ...
    while (eventCount == 0) { 
        /* Make sure that the window system is initialized before the event
         * stream is opened. There is a small window that could possibly
         * result in KVM to crash if the event stream is opened before 
         * the window system is initialized.
         */
        ...
        case ButtonPress:
            type = penDownKVMEvent;
            goto button;
        case ButtonRelease:
            type = penUpKVMEvent;
            goto button;
        case MotionNotify: 
            type = penMoveKVMEvent;
        ...
        button:
            { 
                ...
                if (event.xbutton.window == paintWindow) {
                    StoreKVMEvent(type, 2, (cell)x, (cell)y);
                    return;
                } else { 
                    ...
                    for (i = 0; i < XtNumber(PalmKeys); i++) { 
                        ...
                        if (x >= r->x && x < r->x + r->width && 
                            ...
                            if (button == PALM_pageUp || button == PALM_pageDown
                                || wantPalmSysKeys) {
                                StoreKVMEvent(keyDownKVMEvent, 1, (cell)button);
                                return;
                            }
                            if (button == PALM_power) {
                                StoreKVMEvent(appStopKVMEvent, 0);
                                return;
                            }
                        }
                    }
        ...
        case KeyPress: { 
            ...
            count = XLookupString(&event.xkey, buff, sizeof(buff), 
                                  NULL, &status);
            for (i = 0; i < count; i++) { 
                StoreKVMEvent(keyDownKVMEvent, 1, (cell)buff[i]);
            }
            /* We don't return, since count may be 0.  We rely on the 
             * "for loop" to take care of this
             */
            break;
        }
        ...
        case ClientMessage:
            ...
            if (   event.xclient.message_type == protocolsAtom 
                ...
                && event.xclient.data.l[0] == deleteWindowAtom) {
...
static XtResource resources[] = { 
    { XtNforeground, XtCForeground, XtRPixel, sizeof(Pixel), 
      XtOffset(kauaiApplicationData*, foreground), XtRString, "BLACK" },
    { XtNbackground, XtCBackground, XtRPixel, sizeof(Pixel), 
      XtOffset(kauaiApplicationData*, background), XtRString, "#6A9E73" },
...
/*=========================================================================
 * FUNCTION:      Initialize the window system
 * TYPE:          UI
 * OVERVIEW:      Called when launching the VM
*=======================================================================*/
...
InitializeWindowSystem() {
    ...
    /******** 
     * If we ever get more than a few atoms, we may want to use code
     * like the following.  But it is being deleted for now. 
     *
     *
     * static const char *atomNames[2] = { "WM_DELETE_WINDOW", "WM_PROTOCOLS" };
     * Atom atoms[2];
     * XInternAtoms(display, (char **)atomNames, 2, FALSE, atoms);
     * deleteWindowAtom = atoms[0];
     * protocolsAtom    = atoms[1];
     */

    XSetWMProtocols(display, outerWindow, &deleteWindowAtom, 1);
...
/*=========================================================================
 * FUNCTION:      Finalize the window system
 * TYPE:          UI
 * OVERVIEW:      Called when closing the VM
*=======================================================================*/
    ...
    kauaiXErrorHandler(Display *disp, XErrorEvent *err)
{
    ...
    fprintf(stderr, "X Windows error %s\n", msg);
...
{
    fprintf(stderr, "X Windows I/O Error\n");
    return 0;
}
