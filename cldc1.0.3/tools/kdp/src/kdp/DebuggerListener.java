...
//			acceptSocket.shutdownOutput();
	...
	/*	
                throw new Exception("couldn't find methodinfo for index " +
		    mid);
         */
                System.out.println("Couldn't find methodinfo for index " + mid);
                ps = new PacketStream(this, in.id, Packet.Reply, INVALID_METHODID);
                ps.send();
	    }

            if (code == null) {
                System.out.println("No linenumber table found for class "+ cf.getClassName());
                ps = new PacketStream(this, in.id, Packet.Reply, NOTFOUND_ERROR);
                ps.send();
                return;
/*
                // fake one line of code so the debugger doesn't go nuts
                ps.writeLong(0);
                ps.writeLong(100);
                ps.writeInt(1);
                ps.writeLong(0);
...
*/
        ...
        ps.writeLong(code[ 0 ][ 1 ]);

        // this is not what it's supposed to be:
        ps.writeLong(code[ code.length - 1 ][ 1 ]); 

        ps.writeInt(code.length);
	...
	ClassFile cf;
        ...
        cid = in.readInt(); // class id - we won't need it
        Log.LOG(3, "variable: class id=" + Integer.toHexString(cid)+ "\n");

        mid = in.readInt();
        Log.LOG(3, "variable: method id=" + Integer.toHexString(mid)+ "\n");
   ...
   private void handleByteCode(PacketStream in) {
	...
	Log.LOG(3, "class id=" + Integer.toHexString(cid)+ "\n");

	mid = in.readInt(); // method id
	Log.LOG(3, "method id=" + Integer.toHexString(mid)+ "\n");
