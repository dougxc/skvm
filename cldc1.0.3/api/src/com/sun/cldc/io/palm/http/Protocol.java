...
/**
 * This implements the http protocol for the Palm
 *
 * @author  Nik Shaylor
...
public class Protocol extends ConnectionBase implements StreamConnection {
    ...
    public void open(String name, int mode, boolean timeouts)
        ...
        try {
            ...
            throw new IOException(
                      "inet exception show signal, " +
                      e + " lasterr " + soc.getlasterror());
        }

        try {
            soc.InetOpenSock(SCHEME_HTTP);
        } catch (IOException e) {
            throw new IOException(
                      "inet exception open sock, " +
                      e + " lasterr " + soc.getlasterror());
        }
    ...
    synchronized public InputStream openInputStream() throws IOException {
        ...
        if (postStream == null) { // We are going a GET
            try {
                soc.InetHTTPReqCreateAndSend("GET", null, url, null);
            } catch (IOException e) {
                throw new IOException(
                          "inet exception create-and-send GET, " +
                          e + " lasterr " + soc.getlasterror());
            }
        } else {                 // We are doing a POST
            postStream.close();
        }
    ...
    synchronized public OutputStream openOutputStream() throws IOException {
        ...
        osopen = true;
        postStream = new PrivateOutputStream(this);
        opens++;
    ...
    public void writeAll(byte buf[]) throws IOException {
        try {
            ...
            throw new IOException(
                      "inet exception create-and-send POST, " + 
                      e + " lasterr " + soc.getlasterror());
        }
    }
}
