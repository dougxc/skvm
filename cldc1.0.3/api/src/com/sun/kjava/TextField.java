...
public class TextField {
    ...
    public void paint() {
        ...
        // draw the "...."
        g.drawLine(startText, yPos+titleHeight+2, startText+textWidth,
                   yPos+titleHeight+2, g.GRAY);
        // and the text
    ...
    public boolean pressed(int x, int y) {
        if((x>startText) && (x<startText+textWidth) && (y>yPos) &&
           (y<(yPos+height))) {
    ...
    public void handleKeyDown(int key) {
        if((key == 0x08) || (key == 0x7f)) {
            ...
            if(textW < textWidth) {
                ...
                // We could add an error beep here!
            }
        }
        caret.setPosition(caretPos, yPos);
        paint();
