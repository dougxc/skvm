...
public class Bitmap {
     ...
     * BitmapType in the Palm OS. Thus, it is tightly coupled 
     * to the Palm OS and relies on this type not changing. 
     * This is done for efficiency as well as to reduce the 
     * allocation of a new object each time a bitmap is drawn.
     */
    private short[] data;
    ...
    public Bitmap(short width, byte[] pixels) {
        createBitmap( width, pixels );
    }


    /**
     * Constructor defines the bitmap. The bits of a bitmap are given as a
     * string pattern, each character define the bitmap. 
     * <p>
     * Create an Image object from the bitmap specified in
     * 'pattern'. Pixels may be spec:ified as transparent,
     * or opaque, using the character '_' for transparency,
     * and any other character for opaqueness. Scanline must
     * be a multiple of 8.
     * <p>
     * @param pattern the string pattern for the bitmap.
     * @param scanline the scanline for the pattern.  Must be a multiple of 8.
     */
    public Bitmap(String pattern, int scanline) {

        if (scanline % 8 != 0) throw new IllegalArgumentException();

        int c, buf, len = pattern.length() / 8;
        ...
        createBitmap((short)(scanline / 8), pixel);
    }

    private void createBitmap( short width, byte[] pixels ) {
        ...
        for (dataIndex = 8; dataIndex < data.length; dataIndex++) {
            ...
            data[dataIndex] = 
                (short)(((pixels[pixelIndex] << 8) & 0xFF00) |
