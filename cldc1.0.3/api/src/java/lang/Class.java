...
The following field and methods are required if the KVM is built with
CLASS_INITIALIZATION_IN_JAVA=1.


    // These private functions and fields are used for Class initialization.
    private final static int CLASS_READY = 5;
    private final static int CLASS_ERROR = -1;

    // Simple setters and getters for internal parts of an INSTANCE_CLASS
    // structure. Can't use simple member field mappings as INSTANCE_CLASS
    // structs are potentially relocated into read-only memory (i.e.
    // USESTATIC) and special updating functions are used to modify them.    
    private native Thread getInitializingThread();
    private native void setInitializingThread(Thread thread);
    private native Class getSuperclass();
    private native int getStatus();
    private native void setStatus(int status);

    // The initialize method cannot call a method of another class.
    private native Thread getCurrentThread();

    // There is no mechanism for calling the "<clinit>" method of a class
    // from within Java. 
    private native void runClinit();

    //
    // This method implements the 11 step class initialization process
    // given in the JVM Specification (Second Edition), section 2.17.5.
    //
    // This method must not call a method from any other class
    //
    private void initialize() throws Throwable {
        // Step 1
        synchronized(this) {
            Thread initThread = getInitializingThread();
            // Step 2
            if (initThread != null) {
                if (initThread != getCurrentThread()) {
                    do {
                        try {
                            wait();
                        } catch (InterruptedException e) {}
                    } while ((initThread = getInitializingThread()) != null);
                } else {
                    // Step 3
                    return;
                }
            ...
            // Step 6
            setInitializingThread(getCurrentThread());
        }
        // Step 7
        if (!isInterface()) {
            Class superClass = getSuperclass();
            if (superClass != null && superClass.getStatus() != CLASS_READY) {
                try {
                    superClass.initialize();
                } catch(Throwable ex) {
                    synchronized(this) {
                        setInitializingThread(null);
                        setStatus(CLASS_ERROR);
                        notifyAll();
                    }
                    throw ex;
                }
