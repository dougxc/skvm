 ...
 *
 * @version $Id: Utility.java,v 1.3 2002/07/19 21:04:50 dougxc Exp $
...
public abstract class Utility {
					  ...
					  int index, int length, boolean verbose)
  {
    ...
    Hashtable stackMapTable = null;
    if (stackMap != null) {
        StackMapEntry[] entries = stackMap.getEntries();
        stackMapTable = new Hashtable(entries.length);
        for (int i = 0; i != entries.length; i++) {
            StackMapEntry entry = entries[i];
            stackMapTable.put(new Integer(entry.getByteCodeOffset()), entry);
        }
    }
	...
	if((length < 0) || (i < length)) {
          // insert the stack map entry here
          if (stackMapTable != null) {
              StackMapEntry entry = (StackMapEntry)stackMapTable.get(new Integer(stream.getIndex()));
              if (entry != null) {
                  buf.append("locals = " + entry.typesToString(entry.getTypesOfLocals(), constant_pool)+"\n");
                  buf.append("stack  = " + entry.typesToString(entry.getTypesOfStackItems(), constant_pool)+"\n");
              }
          }
  ...
  public static final String codeToString(byte[] code,
    ...
    return codeToString(code, constant_pool, null, index, length, true);
  }
  ...
  public final static String typeToSignature(String str) {
    ...
    int i;
    for(i=Constants.T_BOOLEAN; i <= Constants.T_VOID; i++) {
      if(type.equals(Constants.TYPE_NAMES[i])) {
	code = Constants.SHORT_TYPE_NAMES[i];
	break;
      }
    }

    if(i == Constants.T_VOID) {
      if(array.startsWith("[")) // Array of void !?
	throw new ClassFormatError("Invalid type: " + str);
    } else if(i > Constants.T_VOID) // Interpret as class name
      code = "L" + type.replace('.', '/') + ";";
