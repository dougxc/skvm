 ...
 *
 * @version $Id: MethodGen.java,v 1.1.1.1 2002/05/21 20:38:49 dougxc Exp $
...
public class MethodGen extends FieldGenOrMethodGen {
  ...
  private Vector          code_attrs_vec  = new Vector();
	private Vector          stack_map_vec   = new Vector();
  ...
  public MethodGen(Method m, String class_name, ConstantPoolGen cp) {
      ...
      if(a instanceof Code)
            ...
            ObjectType    c_type = null;
	    			...
	    			if(type > 0) // contains bla/blubb, but ObjectType() doesn't care
				      c_type = new ObjectType(cp.getConstantPool().
			    ...
			    else if (a instanceof StackMap)
			    {
			      StackMapEntry[] entries = ((StackMap)a).getEntries();
			      for (int k = 0; k < entries.length; k++)
			      {
			        StackMapEntry entry = entries[k];
							InstructionHandle ih = il.findHandle(entry.getByteCodeOffset());
							int[] types_of_locals = entry.getTypesOfLocals();
							int[] types_of_stack_items = entry.getTypesOfStackItems();
							StackMapItem[] locals = new StackMapItem[types_of_locals.length];
							StackMapItem[] stack_items = new StackMapItem[types_of_stack_items.length];
							for (int l = 0; l < locals.length; l++)
								locals[l] = new StackMapItem(types_of_locals[l],this);
							for (int l = 0; l < stack_items.length; l++)
								stack_items[l] = new StackMapItem(types_of_stack_items[l],this);
			        addStackMapEntry(ih,locals,stack_items);
			      }
			    }
			    else
	...
	/**
	 * Give an instruction a stack map corresponding to the runtime state of
	 * the local variables and operand stacks at that instruction.
	 * @param ih the targeted instruction
	 * @param locals the types on the local variables stack at this instruction
	 * @param operands the types on the operand stack at this instruction
	 */
	public StackMapEntryGen addStackMapEntry(InstructionHandle ih, StackMapItem[] locals,
		StackMapItem[] operands)
	{
		StackMapEntryGen gen = new StackMapEntryGen(ih,locals,operands);
		int i;
    if((i = stack_map_vec.indexOf(gen)) >= 0) // Overwrite if necessary
      stack_map_vec.setElementAt(gen, i);
    else
      stack_map_vec.addElement(gen);
    return gen;
	}
   ...
   * Remove a stack map.
   */
  public void removeStackMap(StackMapEntryGen sm) {
    stack_map_vec.removeElement(sm);  
  }
  ...
  /**
   * Sort stack maps by instruction offsets
   */
  private static final void sort(StackMapEntryGen[] maps, int l, int r) {
    int i = l, j = r;
    int m = maps[(l + r) / 2].getInstruction().getPosition();
    StackMapEntryGen h;

    do {
      while(maps[i].getInstruction().getPosition() < m) i++;
      while(m < maps[j].getInstruction().getPosition()) j--;

      if(i <= j) {
        h=maps[i]; maps[i]=maps[j]; maps[j]=h; // Swap elements
        i++; j--;
      }
    } while(i <= j);

    if(l < j) sort(maps, l, j);
    if(i < r) sort(maps, i, r);
  }
	
  /**
   * @return array of stack map entries, sorted by instruction offsets
   */
  public StackMapEntryGen[] getStackMapEntries() {
    int                size = stack_map_vec.size();
    StackMapEntryGen[] lg        = new StackMapEntryGen[size];
    stack_map_vec.copyInto(lg);
  ...
  /**
   * @return `StackMap' attribute of all the stack maps of this method.
   */
  public StackMap getStackMap(ConstantPoolGen cp) {
    StackMapEntryGen[] lg   = getStackMapEntries();
    int           size = lg.length;
    StackMapEntry[] lv = new StackMapEntry[size];

		int length = 2;
    for(int i=0; i < size; i++)
		{
      lv[i] = lg[i].getStackMapEntry(cp);
			length += lg[i].getSize();
		}

    return new StackMap(cp.addUtf8("StackMap"),
				  length, lv, cp.getConstantPool());
  }
  ...
  public Method getMethod() {
    ...
    if((line_number_vec.size() > 0) && !strip_attributes)
      addCodeAttribute(getLineNumberTable(cp));
		...
		/*
		 * Create StackMap attribute
		 */
		if (stack_map_vec.size() > 0)
			addCodeAttribute(getStackMap(cp));
  ...
  public static int getMaxStack(ConstantPoolGen cp, InstructionList il, CodeExceptionGen[] et) {
	...
	if(instruction instanceof Select) {
	  ...
	  if(tag == Constants.JSR || tag == Constants.JSR_W)
	...
	if(tag == Constants.ATHROW || tag == Constants.RET ||
	   (tag >= Constants.IRETURN && tag <= Constants.RETURN))
	  ih = null;
      }
